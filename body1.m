function [D,path,loop,Searchtime]=body1(map,sizeE,Sx,Sy,Ex,Ey)
Pathx=zeros(sizeE);
Pathy=zeros(sizeE);
Pathx(Sy,Sx)=Sx;
Pathy(Sy,Sx)=Sy;
...
for loop=1:100 % iterations
    ...
    if loop>1 % Eqs.(4)and(5) 
        ...
        if numel(find(MinD==new_MinD(end)))>2 % the judgment condition of algorithm convergence (Note that this value can be increased appropriately for higher search accuracy.)
            path=[];
            path(:,1)=Searchx;
            path(:,2)=Searchy;
            break
