...
for i=1:20 
    % map1
    [D1,path1,loop,Searchtime1]=body1(map1,sizeE1,Sx,Sy,Ex1,Ey1);
    path1(30,1)=0;
    Searchpathy{1,1}(:,i)=path1(:,1);
    Searchpathx{1,1}(:,i)=path1(:,2);
    Searchdistance(1,i)=D1(loop,1);
    Searchtime(1,i)=Searchtime1;
    
    % map2
    [D2,path2,loop,Searchtime2]=body1(map2,sizeE1,Sx,Sy,Ex1,Ey1);
    path2(30,1)=0;
    Searchpathy{1,2}(:,i)=path2(:,1);
    Searchpathx{1,2}(:,i)=path2(:,2);
    Searchdistance(2,i)=D2(loop,1);
    Searchtime(2,i)=Searchtime2; 
    
    % map3
    [D3,path3,loop,Searchtime3]=body1(map3,sizeE1,Sx,Sy,Ex1,Ey1);
    path3(30,1)=0;
    Searchpathy{1,3}(:,i)=path3(:,1);
    Searchpathx{1,3}(:,i)=path3(:,2);
    Searchdistance(3,i)=D3(loop,1);
    Searchtime(3,i)=Searchtime3;
    
    % map4
    [D4,path4,loop,Searchtime4]=body1(map4,sizeE2,Sx,Sy,Ex2,Ey2);
    path4(30,1)=0;
    Searchpathy{1,4}(:,i)=path4(:,1);
    Searchpathx{1,4}(:,i)=path4(:,2);
    Searchdistance(4,i)=D4(loop,1);
    Searchtime(4,i)=Searchtime4;
    
    % map5
    [D5,path5,loop,Searchtime5]=body1(map5,sizeE2,Sx,Sy,Ex2,Ey2);
    path5(30,1)=0;
    Searchpathy{1,5}(:,i)=path5(:,1);
    Searchpathx{1,5}(:,i)=path5(:,2);
    Searchdistance(5,i)=D5(loop,1);
    Searchtime(5,i)=Searchtime5; 
    
    % map6
    [D6,path6,loop,Searchtime6]=body1(map6,sizeE2,Sx,Sy,Ex2,Ey2);
    path6(30,1)=0;
    Searchpathy{1,6}(:,i)=path6(:,1);
    Searchpathx{1,6}(:,i)=path6(:,2);
    Searchdistance(6,i)=D6(loop,1);
